 ...
 *
 * Bridges the unified field catalog and assessment engine to the existing
 * UI components. Provides functions to convert between the unified system's
 * types and the legacy Priority Dashboard / Modeling Assistant types.
 *
...
import type {
  ...
  FieldSource,
  SignalContribution,
} from '../../assessment/packages/domain/src/catalog/types';
...
import type { PersonaType } from '../types/priority';
 ...
 */
const LEGACY_FIELD_TO_SIGNALS: Record<MetadataFieldType, SignalType[]> = {
...
export function calculateUseCaseReadiness(
  ...
  const signalScores = useCaseProfile.signals.map((signalWeight) => {
    ...
    const passing = score >= (useCaseProfile.thresholds.partial / 100);
  ...
  if (readinessScore >= useCaseProfile.thresholds.ready / 100 && missingRequirements.length === 0) {
    status = 'ready';
  } else if (readinessScore >= useCaseProfile.thresholds.partial / 100) {
    status = 'partial';
  }
  ...
  for (const signal of signalCoverage) {
    ...
    for (const legacyField of legacyFields) {
      ...
      priorities.push({
        ...
        signalCoverage: signal.coveragePercent,
        workstream: signal.workstream,
      } as Priority & { signalType: SignalType; signalCoverage: number; workstream: string });
    }
  }
...
export function describeFieldSource(source: FieldSource): string {
  switch (source.type) {
    ...
    case 'native_any':
      return `Native attributes (any): ${source.attributes.join(' | ')}`;
    ...
    case 'classification':
      ...
      if (source.anyOf) return `Classifications: ${source.anyOf.join(', ')}`;
...
function getLegacyFieldName(field: UnifiedField): MetadataFieldType {
  ...
  if (source.type === 'native_any') {
    return source.attributes[0] as MetadataFieldType;
  }
...
export function buildFieldEvidence(
  ...
  return {
    ...
    coverage: {
      ...
      assetsCovered: coverage?.assetsCovered || 0,
