/**
 * CustomMetadataDesigner - Visual custom metadata schema builder
 *
 * Features:
...
interface CustomMetadataDesignerProps {
...
export function CustomMetadataDesigner({ initialSchemas, onSave }: CustomMetadataDesignerProps) {
  // Store hooks
  const storeSchemas = useModelStore((state) => state.model.customMetadata || EMPTY_ARRAY);
  const addCustomMetadata = useModelStore((state) => state.addCustomMetadata);
  const updateCustomMetadata = useModelStore((state) => state.updateCustomMetadata);
  const deleteCustomMetadata = useModelStore((state) => state.deleteCustomMetadata);

  // Local state for standalone mode
  const [localSchemas, setLocalSchemas] = useState<CustomMetadataDesign[]>(initialSchemas || []);
  
  // Determine mode
  const isStandalone = !!initialSchemas;
  const schemas = isStandalone ? localSchemas : storeSchemas;

  const [step, setStep] = useState<'template' | 'design'>(schemas.length > 0 ? 'design' : 'template');
  const [selectedSchemaId, setSelectedSchemaId] = useState<string | null>(schemas.length > 0 ? schemas[0].id : null);
  ...
  const handleSelectTemplate = (template: CustomMetadataTemplate) => {
    ...
    if (isStandalone) {
      setLocalSchemas((prev) => [...prev, newSchema]);
    } else {
      addCustomMetadata(newSchema);
    }
  ...
  const updateSchema = (id: string, updates: Partial<CustomMetadataDesign>) => {
    if (isStandalone) {
      setLocalSchemas(schemas.map((s) => (s.id === id ? { ...s, ...updates } : s)));
    } else {
  ...
  const deleteSchema = (id: string) => {
    if (isStandalone) {
      setLocalSchemas(schemas.filter((s) => s.id !== id));
    } else {
    ...
    return (
      <div className="h-full flex flex-col bg-slate-50">
        <div className="flex-1 flex flex-col items-center justify-center p-8">
          ...
          <h2 className="text-2xl font-bold text-slate-900 mb-2">Define Custom Metadata</h2>
          <p className="text-slate-600 text-center max-w-md mb-8">
            Extend Atlan's metadata model with your own business-specific attributes. Choose a template to get started.
          </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl w-full">
            {CUSTOM_METADATA_TEMPLATES.map((template) => {
              ...
              return (
                <button
                  ...
                  className="flex flex-col items-start p-6 bg-white border border-slate-200 rounded-xl hover:border-purple-500 hover:shadow-md transition-all text-left group"
                >
                  <div className={`w-10 h-10 ${colors.bg} ${colors.text} rounded-lg flex items-center justify-center mb-4 group-hover:scale-110 transition-transform`}>
                    <Icon size={20} />
                  </div>
            ...
            <button
              ...
              className="flex flex-col items-start p-6 bg-white border border-slate-200 rounded-xl hover:border-purple-500 hover:shadow-md transition-all text-left group"
            >
              <div className="w-10 h-10 bg-slate-100 text-slate-600 rounded-lg flex items-center justify-center mb-4 group-hover:bg-purple-50 group-hover:text-purple-600 transition-colors">
                <Plus size={20} />
              </div>
              <h3 className="font-bold text-slate-900 mb-1">Start from Scratch</h3>
              <p className="text-xs text-slate-500">Create a completely custom metadata structure.</p>
            </button>
          </div>
        </div>
      </div>
...
export default CustomMetadataDesigner;
