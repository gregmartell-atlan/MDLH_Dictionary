...
import type { SignalType, TriState, UseCaseProfile, UnifiedField } from '../catalog/types';
import type { TenantConfiguration, TenantFieldMapping } from '../discovery/types';
...
import { getEffectiveFieldSource } from '../discovery/tenant-config';
import {
  generateAllRollups,
...
function evaluateSource(
  ...
  switch (source.type) {
    ...
    case 'classification': {
      ...
      const classificationSource = source as { type: 'classification'; pattern?: string; tag?: string };
      const patternStr = classificationSource.pattern || classificationSource.tag || '.*';
      const pattern = new RegExp(patternStr, 'i');
      const hasMatch = asset.classifications.some(c => pattern.test(c));
 ...
 */
function describeSource(source: UnifiedField['source']): string {
  switch (source.type) {
    case 'native':
    ...
    case 'classification': {
      const classSource = source as { type: 'classification'; pattern?: string; tag?: string };
      return `classification:${classSource.pattern || classSource.tag || '*'}`;
    }
    case 'relationship':
...
export async function runAssessment(
  ...
  const dimensions = request.rollupDimensions || ['connection', 'schema', 'owner'];
