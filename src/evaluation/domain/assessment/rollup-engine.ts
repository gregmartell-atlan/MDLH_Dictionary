...
import type { SignalType } from '../catalog/types';
import { groupAssetsByDimension, getDimensionValue } from './scope-resolver';
...
): Record<SignalType, SignalAggregation> {
  const aggregations: Record<SignalType, SignalAggregation> = {} as Record<SignalType, SignalAggregation>;
...
export function generateAllRollups(
  ...
  signalResults: Map<string, AssetSignalResults>,
  useCaseResults: Map<string, AssetUseCaseResults>
): Record<RollupDimension, RollupNode[]> {
  const rollups: Record<RollupDimension, RollupNode[]> = {} as Record<RollupDimension, RollupNode[]>;
...
export function generateHierarchicalRollup(
  ...
  const connectionNodes: RollupNode[] = [];
