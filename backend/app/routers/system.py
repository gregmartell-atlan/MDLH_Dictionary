...
def build_system_config(conn, session_id: str) -> dict:
    ...
    try:
        ...
        # First, ensure we have a database context
        # Query current database - if none, try to set a default
        try:
            cursor.execute("SELECT CURRENT_DATABASE()")
            current_db = cursor.fetchone()[0]
            if not current_db:
                # Try to set a database context
                cursor.execute(f"USE DATABASE {metadata_db}")
                logger.info(f"[{session_id}] Set database context to {metadata_db}")
        except Exception as db_err:
            logger.warning(f"[{session_id}] Could not set database context: {db_err}")
