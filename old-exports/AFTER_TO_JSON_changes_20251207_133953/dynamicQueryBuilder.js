 ...
 * @returns {Object} Categorized tables with metadata
 */
export function analyzeDiscoveredTables(discoveredTables) {
...
export function buildUpstreamLineageQuery(database, schema, processTable, sampleGuid) {
  ...
  if (!sampleGuid || sampleGuid.includes('<') || sampleGuid.includes('>')) {
    console.warn('buildUpstreamLineageQuery: No valid GUID provided');
    return null;
  }
...
SELECT 
    ...
    TO_JSON("INPUTS") AS upstream_assets,
    TO_JSON("OUTPUTS") AS downstream_assets
FROM ${fqn}
WHERE TO_JSON("OUTPUTS") ILIKE '%' || ${safeGuid} || '%'
LIMIT 50;`
  };
}
...
export function buildDownstreamLineageQuery(database, schema, processTable, sampleGuid) {
  ...
  // CRITICAL: Only use real GUIDs - never placeholders
  if (!sampleGuid || sampleGuid.includes('<') || sampleGuid.includes('>')) {
    console.warn('buildDownstreamLineageQuery: No valid GUID provided');
    return null;
  }
...
SELECT 
    ...
    TO_JSON("OUTPUTS") AS downstream_assets
FROM ${fqn}
WHERE TO_JSON("INPUTS") ILIKE '%' || ${safeGuid} || '%'
LIMIT 50;`
  };
}
...
export function buildDynamicRecommendations({
  ...
  const analysis = analyzeDiscoveredTables(discoveredTables);
 ...
 */
export function getAvailableQueryCategories(discoveredTables) {
  const analysis = analyzeDiscoveredTables(discoveredTables);
