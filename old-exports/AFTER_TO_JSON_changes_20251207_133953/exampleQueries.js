...
WHERE TO_JSON(ANCHOR) ILIKE '%<GLOSSARY_GUID>%';`
    },
    {
...
GROUP BY CERTIFICATESTATUS;`
  ...
  FROM ATLASGLOSSARYTERM_ENTITY
  WHERE TO_JSON(ANCHOR) ILIKE '%<CORE_GLOSSARY_GUID>%'
),
non_core_terms AS (
  ...
  FROM ATLASGLOSSARYTERM_ENTITY
  WHERE ANCHOR::VARCHAR NOT ILIKE '%<CORE_GLOSSARY_GUID>%'
),
glossary_lookup AS (
  SELECT GUID AS glossary_guid, NAME AS glossary_name
  FROM ATLASGLOSSARY_ENTITY
...
JOIN glossary_lookup T3
  ON TO_JSON(T2.non_core_anchor_guid) ILIKE '%' || T3.glossary_guid || '%'
...
FROM TABLE_ENTITY
WHERE TO_JSON(columns) ILIKE '%<COLUMN_GUID>%';`
    },
    {
