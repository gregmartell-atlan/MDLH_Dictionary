...
WITH RECURSIVE lineage_chain AS (
    ...
    JOIN PROCESS_ENTITY P ON TO_JSON(P.OUTPUTS) ILIKE '%' || lc.asset_guid || '%'
...
SELECT
    ...
    CERTIFICATESTATUS
FROM TABLE_ENTITY
WHERE TO_JSON(OWNERUSERS) ILIKE '%{{OWNER_USERNAME}}%'
...
SELECT
    ...
    T.QUALIFIEDNAME
FROM TABLE_ENTITY T
WHERE TO_JSON(T.MEANINGS) ILIKE '%{{TERM_GUID}}%'
...
WHERE TO_JSON(C.MEANINGS) ILIKE '%{{TERM_GUID}}%';`,
  },
