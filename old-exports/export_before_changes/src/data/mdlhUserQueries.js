...
WITH RECURSIVE lineage_cte AS (
    ...
    SELECT 
        ...
        1 AS hop_level
    FROM PROCESS_ENTITY P
    WHERE ARRAY_CONTAINS('{{START_GUID}}'::VARIANT, P.INPUTS)
    ...
    SELECT 
        ...
        L.hop_level + 1
    FROM PROCESS_ENTITY P
    JOIN lineage_cte L ON ARRAY_CONTAINS(L.asset_guid::VARIANT, P.INPUTS)
...
FROM ATLASGLOSSARYTERM_ENTITY gt
LEFT JOIN ATLASGLOSSARY_ENTITY g ON gt.ANCHOR = g.GUID
LEFT JOIN ATLASGLOSSARYCATEGORY_ENTITY gc ON ARRAY_CONTAINS(gc.GUID::VARIANT, gt.CATEGORIES)
