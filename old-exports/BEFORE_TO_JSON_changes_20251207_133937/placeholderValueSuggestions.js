  ...
  domain: {
    ...
    query: (db, schema) => `
      SELECT DISTINCT 
        dd."NAME" AS value,
        dd."GUID" AS guid,
        COUNT(DISTINCT dp."GUID") AS asset_count
      FROM ${db}.${schema}.DATADOMAIN_ENTITY dd
      LEFT JOIN ${db}.${schema}.DATAPRODUCT_ENTITY dp 
        ON dp."DOMAINGUIDS"::VARCHAR ILIKE '%' || dd."GUID" || '%'
      WHERE dd."NAME" IS NOT NULL
      GROUP BY dd."NAME", dd."GUID"
      HAVING COUNT(DISTINCT dp."GUID") > 0
      ORDER BY asset_count DESC, dd."NAME"
      LIMIT 50
    `,
    ...
    displayFormat: (row) => ({
      ...
      detail: row.ASSET_COUNT > 0 ? `${row.ASSET_COUNT} products` : 'Domain',
  ...
  glossary: {
    ...
    query: (db, schema) => `
      ...
      LEFT JOIN ${db}.${schema}.ATLASGLOSSARYTERM_ENTITY t 
        ON ARRAY_CONTAINS(g."GUID"::VARIANT, t."ANCHOR")
...
export async function fetchPlaceholderValues(type, database, schema, availableTables, executeQuery) {
  ...
  if (config.sourceTable && !tablesSet.has(config.sourceTable.toUpperCase())) {
    log.debug(`Source table ${config.sourceTable} not found for ${type}`);
    return [];
  }
  ...
  try {
    ...
    try {
      ...
      if (config.fallbackQuery) {
        log.debug(`Primary query failed, trying fallback for ${type}`);
    ...
    if (!result?.rows?.length) {
      log.debug(`No values found for ${type}`);
      return [];
    }
    ...
    const values = result.rows.map(row => {
      ...
      return config.displayFormat(normalizedRow);
    }).filter(v => v.value); // Filter out null values
    
    // Cache results
    placeholderCache.set(cacheKey, values);
    
    log.info(`Fetched ${values.length} values for ${type}`);
    return values;
