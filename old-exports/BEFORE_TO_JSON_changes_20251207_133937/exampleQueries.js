...
GROUP BY CERTIFICATESTATUS;`
  ...
  FROM ATLASGLOSSARYTERM_ENTITY
  WHERE ARRAY_CONTAINS('<CORE_GLOSSARY_GUID>'::VARIANT, ANCHOR)
),
non_core_terms AS (
  ...
  FROM ATLASGLOSSARYTERM_ENTITY
  WHERE NOT(ARRAY_CONTAINS('<CORE_GLOSSARY_GUID>'::VARIANT, ANCHOR))
),
glossary_lookup AS (
  SELECT GUID AS glossary_guid, NAME AS glossary_name
  FROM ATLASGLOSSARY_ENTITY
...
JOIN glossary_lookup T3
  ON ARRAY_CONTAINS(T3.glossary_guid::VARIANT, T2.non_core_anchor_guid)
...
FROM TABLE_ENTITY
WHERE ARRAY_CONTAINS('<COLUMN_GUID>'::VARIANT, columns);`
    },
    {
